...
def test_should_unsuspend_
  <%= file_name %>_with_nil_activation_code_and_activated_at_to_passive_state
    <%= table_name %>(:quentin).suspend!
...
def create_
  ...
  record.<% if options[:stateful] %>register!
              if record.valid?<%
            else
              %>save<% end %>
              record
