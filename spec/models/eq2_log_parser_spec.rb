...
describe Eq2LogParser do
  ...
  context 'instance methods' do
    ...
    describe 'data retrieved from drops' do
      let(:drop_list) {
        [
            '(1316254470)[Sat Sep 17 20:14:30 2011] You loot \aITEM 842968069 ' +
                '-1475883379:Pure Primal Velium Shard\/a from the Exquisite Chest of Prime-Cornicen Munderrad1.',
            '(1316254474)[Sat Sep 17 20:14:34 2011] You loot \aITEM 1371925287 ' +
                '183231148:Thornskin VIII (Master)\/a from the Exquisite Chest of Prime-Cornicen Munderrad2.',
            '(1316254535)[Sat Sep 17 20:15:35 2011] Spyce loots \aITEM 595219945 ' +
                '-1154546896:Wand of the Kromzek Warmonger\/a from the Exquisite Chest of Prime-Cornicen Munderrad3.'
        ]
      }
      ...
      context '#drops' do
        context 'includes drops' do
          it 'from the start of the log section' do
            File.any_instance.stub(:grep).and_return(drop_list)

            expect(subject.drops(1316254470, 1316254474)).to eq drop_list.first(2)
          end

          it 'from the end of the log section' do
            File.any_instance.stub(:grep).and_return(drop_list)

            expect(subject.drops(1316254474, 1316254535)).to eq drop_list.last(2)
          end

          it 'from the middle of the log section' do
            File.any_instance.stub(:grep).and_return(drop_list)

            expect(subject.drops(1316254470, 1316254535)).to eq drop_list
          end
        end
      end
      ...
      context '#mobs' do
        context 'includes mobs' do
          it 'from the start of the log section' do
            ...
            expect(subject.mobs(1316254470, 1316254474)).
                to eq ['Prime-Cornicen Munderrad1', 'Prime-Cornicen Munderrad2']
          end

          it 'from the end of the log section' do
            File.any_instance.stub(:grep).and_return(drop_list)

            expect(subject.mobs(1316254474, 1316254535)).
                to eq ['Prime-Cornicen Munderrad2', 'Prime-Cornicen Munderrad3']
          end

          it 'from the middle of the log section' do
            File.any_instance.stub(:grep).and_return(drop_list)

            expect(subject.mobs(1316254470, 1316254535)).
                to eq ['Prime-Cornicen Munderrad1', 'Prime-Cornicen Munderrad2', 'Prime-Cornicen Munderrad3']
          end
        end
      end
    end
