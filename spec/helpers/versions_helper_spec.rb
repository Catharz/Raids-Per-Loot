  ...
  describe '#changes' do
    context 'updates' do
      it 'excludes the updated_at column by default' do
        ...
        version = FactoryGirl.create(:version, item: char2, event: 'update', object: char1.to_yaml)

        changes(version).should eq [{key: 'id', old: char1.id, new: char2.id},
      ...
      it 'excludes others columns when requested to' do
        ...
        changes(version, excluded).should eq [{key: 'name', old: char1.name, new: char2.name}]
      end
    ...
    context 'creates' do
      it 'excludes the updated_at column by default' do
        char2 = FactoryGirl.create(:character)
        version = FactoryGirl.create(:version, item: char2, event: 'create', object: nil)

        changes(version).should eq [{key: 'id', old: nil, new: char2.id},
      ...
      it 'excludes others columns when requested to' do
        ...
        excluded = Character.attribute_names - %w{name}
        changes(version, excluded).should eq [{key: 'name', old: nil, new: char2.name}]
      end
    ...
    context 'deletes' do
      it 'excludes the updated_at column by default' do
        char1 = FactoryGirl.create(:character, confirmed_rating: 'optimal', confirmed_date: Date.today)
        version = FactoryGirl.create(:version, item: nil, item_id: char1.id, item_type: 'Character', event: 'destroy', object: char1.to_yaml)
        version.should_receive(:reify).and_return(char1)

        changes(version).should eq [{key: 'id', old: char1.id, new: nil},
      ...
      it 'excludes others columns when requested to' do
        ...
        excluded = Character.attribute_names - %w{name}
        changes(version, excluded).should eq [{key: 'name', old: char1.name, new: nil}]
      end
