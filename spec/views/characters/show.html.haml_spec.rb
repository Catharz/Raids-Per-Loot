...
describe 'characters/show.html.haml' do
  ...
  let(:character) { mock_model(Character,
                               FactoryGirl.attributes_for(:character).
                                   merge!(name: 'Julie',
                                          archetype: coercer)) }
  let(:characters_data) { YAML.load_file File.join(Rails.root, 'spec',
                                                   'fixtures',
                                                   'soe',
                                                   'soe_characters.yml') }
  let(:armour) { mock_model(LootType, name: 'Armour') }
  let(:jewellery) { mock_model(LootType, name: 'Jewellery') }
  let(:raids) { Array.new(2) { |n| mock_model(Raid,
                                              raid_date: Date.today + n.days)
  } }
  let(:instances) { Array.new(3) { |n| mock_model(Instance,
                                                  raid: raids[n % 2]) } }
  ...
  before(:each) do
    ...
    drop_list = create_drops(character,
                             'Armour' => 2,
                             'Weapon' => 3,
                             'Jewellery' => 1)
    ...
    external_data =
        mock_model(ExternalData,
                   ...
                   data: characters_data.
                       with_indifferent_access[:character_list][0])
